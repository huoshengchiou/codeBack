...
const columnsFormat = numberOfGame => {

    const gameObjList = [...new Array(numberOfGame)]
        .map((_, index) => (
            {
                title: `Game ${index + 1}`,
            ...
            render: (_, data) => {
                ...
                return (
                    <div className={cx('statusBlock')}>
                        {
                            rankingRise > 0 ?
                                <img src={require('../imgs/ico_rise.svg')}
                                    alt='ico_rise' /> :
                                <img src={require('../imgs/ico_down.svg')}
                                    alt='ico_down' />
                        }
...
const Standings = props => {
    const { authData, configData, match } = props
    ...
    const getStageInfo = () => {
        ...
        from(apiWithTokenWrapper(getStandingInfo, data))
            .subscribe(res => {
    ...
    const getStandingDetail = stage_no => {
        ...
        from(apiWithTokenWrapper(getGameStandingStageInfo, data))
            .subscribe(res => {
                const { data, status } = res;
                const { body, header } = data
                if (status === 200 && header.status === 'OK') {
                    ...
                    const groupTable = groupList.map(groupData => {
                        ...
                        return {
                            title: `Group ${getGroupName(groupData.group_no - 1)}`,
                            status: groupData.group_status === 'completed',
                            data,
                        }
                    })
                    setColumns(columnsFormat(game_amount))
        ...
        <div className={cx('box')}>
            ...
            <div className={cx('tabContainer')}>
                ...
                <div className={cx('rightCol')}>
                    <TabLv2 theme="dark" tabList={tabList}
                            ...
                            <>
                                {
                                    ...
                                    <div className={cx('cardContainer')}
                                        key={`AwardPodium_${index}`}>
                                        <AwardPodium
                                            rank={item.data.slice(0, 3)}
                                ...
                                <div className={cx('row')}
                                    ...
                                    <div className={cx('titleBar')}>
                                        ...
                                        {
                                            item.status ?
                                                <span className={cx('green')}>Completed</span> :
                                                <span className={cx('yellow')}>Waiting for Result</span>
                                        }
                                    </div>
                                    {
                                        (item.data.length === 0) ?
