...
const Registration = ({ search, setIsRegistered }) => {
  ...
  const players = participants?.map(o => <PlayerCard key={o.t8t_player_id} {...o} authData={authData} intl={intl} />);

  const teams = participants?.map(o => <TeamCard key={o.t8t_team_id} {...o} authData={authData} intl={intl}/>);
