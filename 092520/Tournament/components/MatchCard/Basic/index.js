...
const MatchCard = (props) => {
  const { offShare = false, status, data, configData, location, history, closePopWindowFunc, openPopWindowFunc, dialogData, intl, authData, t8t_lite } = props;
  const { isLoggedIn, profile } = authData;
  const { openDialogFunc } = dialogData
  const { getImageUrl, pathPrefix } = configData;
  const { t8t_serial, banner_image } = data.t8t_lite;
    ...
    switch (status) {
      ...
      case "waiting_visitor_verified":
        switch (vistorHomeStatus) {
          ...
          case 3:
            return <div>
              <Button title={intl.formatMessage({ id: 'Submit-Result_[btn]Confirm Result' })} onClick={() => goBattleRoom()} />
            </div>
            return ""
          default:
            return ""
  ...
  const Sharebox = () => { return <> <Share ShareLink={`${pathPrefix}/tournamentEmbed/matchcard/${data.bracket_elimination_match_id}`} TitlePos='center' Needlayers={['L1', 'L2', 'L3']} ShareOrder={['Facebook', 'Twitter']} BannerSrc={getImageUrl(banner_image)} /></> }
  const goShare = () => {
    openDialogFunc({
      component: Sharebox,
      componentProps: {
          ...
          <div className={classes.leftCol}>
            <div className={classes.teamName}>
              ...
              <p>
                ...
                {data.visitor_name === null ? "--" : data.visitor_name}</p>
            </div>
