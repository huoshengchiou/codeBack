...
//images
import ico_add from 'images/TournamentJoin/ico_add.svg';
import ico_trash from 'images/TournamentJoin/ico_trash.svg';
...
const SelectTeam = () => {
    ...
    const AddStarter = () => {
        ...
        useEffect(() => {
            const arr1 = starterarr.map(val => {
                return val.member_id;
            });
            const arr2 = bencharr.map(val => {
                return val.member_id;
            });
            ...
            <>
                {FetchMemberFinish ? (
                    <div className={classes.addStarterBox}>
                        ...
                        <p className={cx('requiredNumber')}>
                            {starterarr.length + tempselect.length} / {needStarterNum}
                        </p>
                        <div>
                                    ...
                                    {digestrenderlist.map((val, idx) => (
                                        <tr key={idx}>
                                            ...
                                            <td
                                                style={{
                                                    opacity: `${
                                                        (val.CheckGId && !val.IsIdVerfied) ||
                                                        (val.CheckCb && !val.IsInClub) ||
                                                        FilterLabelarr.indexOf(val.member_id) !== -1
                                                            ? '0.5'
                                                            : '1'
                                                    }`
                                                }}
                                            >
                                                ...
                                                <Checkbox
                                                    title=""
                                                    checked={
                                                        tempselect.indexOf(val.member_id) !== -1 ||
                                                        FilterLabelarr.indexOf(val.member_id) !== -1
                                                    }
                                            ...
                                            >
                                                ...
                                                {/* <img alt="" src={getImageUrl(val.profile_picture)} alt="" /> */}
                                                <Thumbnail
                                                    border={{ gap: 2 }}
                                                    size="49px"
                                                    imgUrl={getImageUrl(val.profile_picture)}
                                                />
                                                <div className={classes.userInfo}>
                                                    ...
                                                    <span className={classes.gameID}>
                                                        {val.inGameId}
                                                        {val.IsIdVerfied && (
                                                            <Icon name={'Verifiation'} customClass={classes.icon} />
                                                        )}
                                                    </span>
                                                </div>
                                            </td>
                                            ...
                                            <td className={classes.userStatus}>
                                                ...
                                                {val.CheckGId &&
                                                    !val.IsIdVerfied &&
                                                    intl.formatMessage({ id: 'TournamentJoin_Not Verified ID' })}
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
            ...
            <>
                {FetchMemberFinish ? (
                    <div className={classes.addStarterBox}>
                        ...
                        <p>{intl.formatMessage({ id: 'TournamentJoin_Required' })}</p>
                        <p className={cx('requiredNumber')}>
                            {bencharr.length + tempselect.length} / {needBenchNum}
                        </p>
                        <div>
                            <table className={cx('addUserTable')}>
                                <thead>
        ...
        <>
            <div className={cx('addStarterCard', 'addNewUser')}>
                <div className={cx('gap')}>
                    ...
                    >
                        <div className={classes.userInfo}>
                            {FetchMemberFinish ? <img src={ico_add} alt="" /> : <Loading theme="dark" />}
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className={cx('infoInner', 'cardBottom')}>
                ...
                <div className={cx('col')}>
                    ...
                    <p className={cx('status')}>
                        {starterarr.length} / {needStarterNum}
                    </p>
                                ...
                                <div className={cx('gap')}>
                                    <div className={classes.column}>
                                        ...
                                        <div className={classes.tools}>
                                            <img
                                                dataui={val.member_id}
                                                src={ico_trash}
                                                alt=""
                                                onClick={() => removeStarter(val)}
                                            />
                                        </div>
                                    </div>
                                </div>
                ...
                <div className={cx('col')}>
                    ...
                    <div className={cx('threebtnArea')}>
                        <ul>
                            <li>
                                <Button
                                    title={intl.formatMessage({ id: 'TournamentJoin_[btn]Discard' })}
                                    theme="dark_2"
                                    onClick={() => {
                                        hadeleDiscardBtn();
                                    }}
                            ...
                            <li>
                                <Button
                                    title={intl.formatMessage({ id: 'TournamentJoin_[btn]Do it later' })}
                                    theme="dark_2"
                                    customClass={classes.secondBtn}
                                    onClick={() => {
                                        teamhandleDIL({
                                            popWindowData,
                                            gameRegEtime,
                                ...
                                {/* TODO根據條件判斷4 5 */}
                                <Button
                                    title={intl.formatMessage({ id: 'TournamentJoin_[btn]Next' })}
                                    //原上限5  //原始bench限制|| (bencharr.length < needBenchNum)
                                    theme="dark_2"
                                    disabled={starterarr.length !== needStarterNum}
                                    customClass={classes.secondBtn}
                                    onClick={() => {
                                        handlenextbtn();
                                    }}
