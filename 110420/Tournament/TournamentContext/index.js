...
export const TournamentContainer = (props) => {
    ...
    //rating fetch key
    const t8tId = useRef(null)
    // rating qualify
    const needRating = useRef(null)
    //finish rating//use when needRating=true 
    ...
    const {
        ...
        ckEndToOngo,
        ckNotOpen,
    } = useGetT8TDetail({ t8t_serial, apiWithTokenWrapper, ProcessFch, history, configData, location, });
    ...
    useEffect(() => {
        ...
        setRegId(GamerRegId);
        tournamentName.current = FchBkT8tInfo.t8t_lite.name
        t8tId.current = FchBkT8tInfo.t8t_id
        needRating.current = FchBkT8tInfo.myself.need_rating
        Rated.current = FchBkT8tInfo.myself.is_rated
    ...
    const getUseGetRank = (gameData) => {
        const { game } = renderT8tInfo.t8t_lite;
        const badge = rankFunc(game.game_code, gameData, true);
        return badge
    }
