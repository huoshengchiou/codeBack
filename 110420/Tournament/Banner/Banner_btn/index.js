...
import {
    PopWindow_V2,
    withPopWindowProvider_V2,
    PopWindowStorage,
    withPopWindowConsumer as withPopWindowConsumer_V2
} from 'components/DesignSystem/PopWindow_V2';
...
import NavLink from 'components/DesignSystem/Nav/Link';
...
//adjustStyle for topBar
export const Banner_btn = ({ size, adjustStyle = false, popWindowData, dialog_V2Data }) => {
    const {
        setProcessFch,
        configData,
        history,
    ...
    //     openPopWindow({
    //         title: 'Rating',
    //         component: (<><RatingCard t8tName={tournamentName.current} type_id={t8tId.current} urlAfterRating={`${configData.pathPrefix}/tournament/list/${GameSerial}/home`} /></>),
    //     })
    //     // 'Warning', 'Error', 'Info', 'Success'
    //     openDialog_V2Func({ title: 'Rating', message: '', type: 'Info', buttons: [] })
    // }, [])
    ...
    useEffect(() => {
        ...
        if (renderT8tInfo.participant_limit - renderT8tInfo.participants_count <= 0 && VisitorStatus === null) {
    ...
    const RegOpenBasicSkin = () => {
        ...
        //非null時為觀察spot數量關閉報名
        if (renderT8tInfo.participant_limit - renderT8tInfo.participants_count <= 0) {
    ...
    const RegOpenSkin = () => {
        ...
        if (
            renderT8tInfo.participant_limit !== null &&
            renderT8tInfo.participant_limit - renderT8tInfo.participants_count <= 0
        ) {
            // return classes.btn_disable;
            // return setIsBtnClose(true)
            IsBtnClose.current = true;
    ...
    const handleRO_B = () => {
        ...
        if (!VisitorStatus) {
            ...
            if (renderT8tInfo.is_need_in_game_id_verified && !renderT8tInfo.myself.is_in_game_id_verified) {
                openDialog_V2Func({
                    title: intl.formatMessage({ id: 'Single-Tournament-Banner-Status_Please Verify Your Game ID' }),
                    message: intl.formatMessage({
                        id: 'Single-Tournament-Banner-Status_[content]Please Verify Your Game ID'
                    }),
                    type: 'Warning',
                    btnsDisplay: 'vertical',
                    buttons: [
            ...
            if (renderT8tInfo.open_rule !== 'public' && !renderCbInfo.is_member) {
                openDialog_V2Func({
                    ...
                    message: `${intl.formatMessage({
                        id: 'Single-Tournament-Banner-Status_[content]Club Member Only'
                    })} ${renderCbInfo.name}`,
                    type: 'Warning',
                    btnsDisplay: 'vertical',
                    buttons: [
    ...
    useEffect(() => {
        ...
        //完成CKin提示
        openDialog_V2Func({
            title: ckInOK.current
                ? intl.formatMessage({ id: 'TournamentJoin_Check-in successfully !' })
                : intl.formatMessage({ id: 'TournamentJoin_Check-in failed !' }),
            ...
            <>
                <div className={classes.lineupListWrapper}>
                    <ul className={classes.lineupList}>
                        {teamParticipateList.current &&
                            teamParticipateList.current.map((val, idx) => {
                                return (
                                    <li className={classes.lineupUnit} key={idx}>
                                        <div className={classes.Ava}>
                                            <Thumbnail
                                                size="53px"
                                                border={{ double: true, gap: 4 }}
                                                imgUrl={getImageUrl(val.icon_image)}
                                            />
                                        </div>
                                        ...
                                        <div className={classes.Status}>
                                            {val.t8t_checkin_at && (
                                                <span>{intl.formatMessage({ id: 'TournamentJoin_Checked-in' })}</span>
                                            )}
                                        </div>
                                    </li>
                                );
                    ...
                    <div className={classes.lineupBtnWrapper}>
                        <Button
                            title="Continue"
                            customClass={classes.continueBtn}
                            onClick={() => closePopWindowFunc()}
                        />
                    </div>
                </div>
            </>
    ...
    const openRating = e => {
        e.preventDefault();
        openPopWindow({
            title: intl.formatMessage({ id: 'T8T-111_Rating' }),
            component: (
                <>
                    <RatingCard
                        callbackAfterRating={() => setProcessFch(true)}
                        rateTitle={tournamentName.current}
                        type="t8t"
                        type_id={t8tId.current}
                        urlAfterRating={`${configData.pathPrefix}/tournament/list/${GameSerial}/home`}
                    />
                </>
            )
        });
    ...
    const RegCloseOngoTask = (
        <>
            {(GameStatus === 'registration_closed' || GameStatus === 'ongoing') && VisitorStatus ? (
                <>
                    <Button
                        title={RenderRegStatus || RenderGameStatus}
                ...
                <>
                    {/* <div className={cx('BtnDisplayBanner', (GameStatus === 'ongoing') && 'OnGo')} style={{ paddingLeft: `${adjustStyle === 'true' ? '20px' : '4px'}`, fontSize: (adjustStyle === 'true') && '16px' }}></div> */}
                    <div
                        className={cx('BtnDisplayBanner', GameStatus === 'ongoing' && 'OnGo')}
                        style={{ fontSize: adjustStyle && '16px' }}
                    >
    ...
    return (
        <>
            ...
            <div style={{ position: 'relative', padding: adjustStyle ? '' : '0 30%' }}>
                ...
                {Toggle && (
                    <>
                        <div
                            className={`${classes.DisCardBtn} ${adjustStyle && classes.adjust}`}
                            onClick={() => {
                                teamhandleDd();
                            }}
                        >
                ...
                {(GameStatus === 'registration_closed' || GameStatus === 'ongoing') && RegCloseOngoTask}
                    ...
                    <>
                        <div className={classes.Tournamentcompeleted}>
                            ...
                            {/* {needRating.current &&
                                (!Rated.current ? (
                                    <>
                                        <div className={classes.compeletedBtnWrapper}>
                                            <Button
                                                title={intl.formatMessage({ id: 'T8T-111_Rating' })}
                                                // disabled={true}
                                                onClick={openRating}
                                    ...
                                    <div>
                                        <NavLink onClick={e => openRating(e)}>
                                            {intl.formatMessage({ id: 'T8T-111_See My Rating' })}
                                        </NavLink>
                                    </div>
                                ))} */}
                        </div>
