...
const MatchCard = (props) => {
  ...
  const Sharebox = () => { return <> <Share ShareLink={`${configData.pathPrefix}/tournamentEmbed/matchcard/${data.bracket_elimination_match_id}`} TitlePos='center' Needlayers={['L1', 'L2', 'L3']} ShareOrder={['Facebook', 'Twitter']} BannerSrc={getImageUrl(data.t8t_lite.banner_image)} titleText={intl.formatMessage({ id: 'Manage-Tournament-Page_Share' })} /></> }
  const goShare = () => {
    openDialogFunc({
      component: Sharebox,
      componentProps: {
      ...
      <div className={classes.prePageContainer}>
        <div className={classes.session}>
          <div className={classes.gameIcon}>
            {data.t8t_lite?.game.icon_image === null ? "" : <img alt="" src={getImageUrl(data.t8t_lite?.game.icon_image)} alt="" />}
              ...
              <div className={classes.teamName}>
                <div className={classes.clubImg}>
                  <div className={classes.inner}>
                    ...
                    {/* {paticipant?.avatar ? <img alt="" src={getImageUrl(paticipant?.avatar.logo_image)} alt="" /> : ""} */}
                  </div>
                </div>
                <p>
              ...
              <div className={classes.teamUserImg}>
                {data.participant.team_players?.slice(0, 5).map((item, index) => {
                  return <Thumbnail
                    ...
                    key={index}
                  />
                  // return <img alt="" key={index} src={getImageUrl(item.icon_image)} alt="" />
                })}
        ...
        <div className={classes.toolsBtn}>
          {getDisplayScore(data.status) ?
            <div className={classes.share} style={{ display: `${offShare ? "none" : ""}` }} onClick={() => goShare()}>
              <img alt="" src={require('../imgs/ico_share.png')} alt="" />
            </div> : ""}
        </div>
      </div>
    </div>
