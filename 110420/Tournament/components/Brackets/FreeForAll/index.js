...
const FreeForAll = (props) => {
  ...
  const { intl, match, authData, configData } = useContext(TournamentContext);
  const { apiWithTokenWrapper, profile, isLoggedIn } = authData;
  ...
  const sortKeyList = [
    ...
    {
      sortKey: 'content',
      intlKey: 'Brackets_Content',
      sortable: false,
      width_pct: "51%"
    },
    {
      sortKey: 'score',
      intlKey: 'Brackets_Score',
      sortable: false,
      width_pct: "20%"
    },
    {
  ...
  const formatData = (data) => {
    return {
        ...
        data.is_winner ?
          <>
            <div className={cx('ico')}>
              <img alt="" src={require('../imgs/ico_ranking.png')} alt='' />
              {/* <Icon name="PinTop" /> */}
            </div>
          </> : "",
      seed: data.seed,
      ...
      icon:
        <>
          <img alt="" src={require('../imgs/arrow.svg')} />
        </>
      ,
      rowOnClick: () => {
        goMagicCard(data)
                      ...
                      <div className={cx('row')} key={index}>
                        <div className={cx('gap')}>
                          <div className={cx('groupCard')}>
                            <h4>{intl.formatMessage({ id: 'Brackets_Group' }, { groupname: getGroupName(index) })} </h4>
